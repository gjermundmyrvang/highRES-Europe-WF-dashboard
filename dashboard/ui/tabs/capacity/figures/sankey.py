...
def render_sankey(df, unit_label):
    ...
    link_colors = ["rgba(100,100,100,0.3)"] * n_techs  # Total Potential → each tech
    ...
    fig = go.Figure(
        go.Sankey(
            node=dict(
                label=labels,
                hovertemplate=f"%{{label}}: %{{value:.1f}} {unit_label}<extra></extra>",
                color=node_colors,
            ),
            link=dict(
                source=source,
                target=target,
                value=value,
                hovertemplate=f"%{{source.label}} → %{{target.label}}: %{{value:.1f}} {unit_label}<extra></extra>",
                color=link_colors,
            ),
